...
// --- Explicit Default Plan ---
// Ensure this object is fully defined in your code
const defaultPlan = {
  Monday: {
...
function App() {
  ...
  const [selectedDay, setSelectedDay] = useState(() => {
      ...
      return localStorage.getItem("selectedDay") || days[todayIndex];
  });
  ...
  // --- Derived State ---
  // Ensure activePlan and activeGroceries always fall back to valid default objects
  const activePlan = dynamicPlan ?? defaultPlan;
  const activeGroceries = dynamicGroceries ?? defaultGroceries;

  // Add safety checks when accessing properties based on selectedDay
  const currentDayPlan = activePlan?.[selectedDay] ?? {}; // Use optional chaining and nullish coalescing for safety
  const meals = currentDayPlan.meals || [];
  const fitness = currentDayPlan.fitness || "";
  const checkedItems = checkedItemsByDay?.[selectedDay] || {}; // Optional chaining for safety
      ...
      <Container sx={{ pt: 2, pb: 4 }}>
        ...
        <Box mt={4}>
          ...
          <Grid container spacing={2}>
             {/* Use derived 'activeGroceries' variable */}
